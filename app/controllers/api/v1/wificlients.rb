...
def update_wireless_clients(params)
  ...
  stations.each do |station|
    ...
    if existing_record.nil? || existing_record.tx_bytes.to_i < station_params['tx bytes'].to_i || existing_record.rx_bytes.to_i < station_params['rx bytes'].to_i
      ...
      # Update existing record if the new data has higher TX or RX bytes
	    if station_params['connected time'] > existing_record.connected_time.to_i 
		    stationTest= stationTest.update(manager_id: User.find_by(id: stationParams['user_id']).manager_id)
	    end
